...
const parseAuthCookieFromHeaders = async (
  headers: Headers,
): Promise<{ secure: boolean; value: JWT | null; name: string } | null> => {
...
const mutatResponseToRemoveAuthJsCookie = (
  ...
  response.cookies.has(SECURE_AUTHJS_COOKIE_NAME) &&
    response.cookies.delete(SECURE_AUTHJS_COOKIE_NAME)
  response.cookies.has(AUTHJS_COOKIE_NAME) &&
    response.cookies.delete(AUTHJS_COOKIE_NAME)
  response.cookies.has('payload-token') &&
...
const validateJwtTokenAndLogoutOnFailure = async (
  ...
  if (
    (authCookie?.value == null &&
      request.cookies.has(authCookie?.name || '')) ||
    (authCookie?.value?.exp != null &&
      !isWithinExpirationDate(new Date(authCookie?.value.exp * 1000)))
  ) {
...
const validateAuthRoutes = async (
  request: NextRequest,
): Promise<NextResponse | true> => {
  const authCookie = await parseAuthCookieFromHeaders(request.headers)
  ...
  if (
    !protectedRoutes.some(route =>
      match(route, { decode: decodeURIComponent })(request.nextUrl.pathname),
    )
  )
    return true
  ...
  if (!authCookie || authCookie?.value == null) {
    const signInUrl = new URL(
      authConfig?.pages?.signIn || '/sign-in',
      request.url,
    )
